...
export default function SubmitIncident() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-black">Ingest Multimodal Incident</h1>
      ...
      {successInfo && (
        <div className="p-5 bg-green-50 border border-green-200 text-green-700 rounded-xl space-y-2 text-xs font-mono">
          <h4 className="font-extrabold text-sm flex items-center gap-1.5 text-green-800">
            <span className="w-2 h-2 bg-green-500 rounded-full pulse-glow"></span>
            Ingestion Dispatched Successfully
          </h4>
          <p>Incident generated with unique ID: {successInfo.incident_id}</p>
          <p className="text-gray-600">Classified Severity: <span className="text-black uppercase font-bold">{successInfo.severity}</span></p>
          <p className="text-gray-500">Redirecting to incident console in 3 seconds...</p>
        </div>
      )}

      <form onSubmit={handleSubmit} className="glassmorphism p-6 rounded-xl border border-gray-200 space-y-6 shadow-sm">
        {/* Format Selector */}
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full bg-white border border-gray-200 text-sm font-mono text-black rounded-lg px-3 py-2.5 focus:outline-none focus:border-blue-500 cursor-pointer shadow-sm"
          >
        ...
        <div className="space-y-4">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider font-mono block">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {file ? (
                <div>
                  <p className="text-sm font-bold text-black truncate max-w-md mx-auto">{file.name}</p>
          ...
          <div className="space-y-1">
            <textarea
              ...
              rows={6}
              className="w-full bg-white border border-gray-200 text-xs font-mono text-black rounded-lg p-3.5 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            />
            {file && (
        ...
        <div className="pt-4 border-t border-gray-200 flex items-center justify-end gap-3">
          <button
            ...
            disabled={submitting}
            className="px-4 py-2 border border-gray-200 bg-white hover:border-gray-300 text-gray-600 hover:text-black rounded-lg text-xs font-semibold shadow-sm transition disabled:opacity-40"
          >
            Clear Fields
          </button>
          ...
          <button
            ...
            className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-semibold shadow-sm transition duration-150 flex items-center gap-2 disabled:opacity-40"
          >
            {submitting ? (
              <>
                <div className="w-3.5 h-3.5 border-2 border-t-white border-r-transparent border-gray-200 rounded-full animate-spin"></div>
